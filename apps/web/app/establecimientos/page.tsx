    ...
    <div className="grid gap-6">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {items.map((e) => (
            <Link key={e.id} href={`/establecimiento/${e.id}`} className="rounded-2xl border border-white/10 bg-white/5 p-5 hover:border-white/30">
              ...
              <p className="mt-3 text-sm text-white/60 line-clamp-2">{e.description || "Establecimiento recomendado para clientes."}</p>
              <div className="mt-3 text-xs text-white/50">
                {e.distance ? `${e.distance.toFixed(1)} km` : "Sin distancia"} • {e.address}
              </div>
            </Link>
          ))}
          {!items.length ? <div className="card p-6 text-white/60">No hay establecimientos con estos filtros.</div> : null}
        </div>
      )}
    </div>
  );
